...
void AstDumper::visit(VariableNode &p_variable) {
    ...
    std::printf("variable <line: %u, col: %u> %s %s\n",
                ...
                p_variable.getNameCString(), p_variable.getType().c_str());
...
void AstDumper::visit(ConstantValueNode &p_constant_value) {
    ...
    std::string type = p_constant_value.getType();
    
    if(type=="integer"){
        std::printf("%d\n",p_constant_value.getInt());
    }else if(type=="real"){
        std::printf("%f\n",p_constant_value.getDouble());
    }else if(type=="string"){
        std::printf("%s\n",p_constant_value.getString());
    }else if(type=="boolean"){
        std::printf("%s\n",p_constant_value.getBool() ? "true" : "false");
    }
}
...
void AstDumper::visit(FunctionNode &p_function) {
    ...
    std::printf("function declaration <line: %u, col: %u> %s %s\n",
                ...
                p_function.getNameCString(), p_function.getALLType());
